...
os.environ.setdefault('THEANO_FLAGS',
                      'blas.ldflags=-lopenblas -L/home/rolustech/lib -lgfortran,floatX=float32,device={},force_device=True,allow_gc=True,print_active_device=False,int_division=floatX'.format(
                          args.device))
...
class DataLoader(threading.Thread):
    ...
    def __init__(self):
        ...
        self.orig_shape, self.seed_shape = args.batch_shape, args.batch_shape
    ...
    def add_to_buffer(self, f):
        ...
        seed = seed.resize((orig.size[0], orig.size[1] ), resample=PIL.Image.LANCZOS )
        ...
        if num_fractions == 0: num_fractions = math.floor(seed.shape[0] / self.seed_shape) * math.floor(
            seed.shape[1] / self.seed_shape)
...
class NeuralEnhancer(object):
    ...
    def process(self, original):
        ...
        # s = 2 ** (args.generator_upscale - args.generator_downscale )
        s = 1
        original = original.resize((original.size[0] * args.zoom, original.size[1] * args.zoom ), resample=PIL.Image.LANCZOS )
        by, bx = original.size[0] % s, original.size[1] % s
        ...
        z = 1
        image = np.pad(original, ((p, p), (p, p), (0, 0)), mode='empty')
